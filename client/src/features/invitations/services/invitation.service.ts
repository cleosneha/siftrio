import { api } from "@/lib/api";
import type { ApiResponse, Invitation, PendingInvitation } from "@/types";

export const invitationService = {
  async invite(resourceType: string, resourceId: string, email: string, role: string) {
    const res = await api.post<ApiResponse<Invitation>>(`/invitations?resource_type=${resourceType}&resource_id=${resourceId}`, { email, role });
    return res.data;
  },

  async listPending(resourceType: string, resourceId: string) {
    const res = await api.get<ApiResponse<PendingInvitation[]>>(`/invitations?resource_type=${resourceType}&resource_id=${resourceId}`);
    return res.data;
  },

  async withdraw(invitationId: string) {
    const res = await api.delete<ApiResponse<Invitation>>(`/invitations/${invitationId}`);
    return res.data;
  },

  async accept(token: string) {
    const res = await api.post<ApiResponse<{ id: string }>>(`/invitations/accept/${token}`);
    return res.data;
  },
};
